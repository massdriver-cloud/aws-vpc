locals {
  ...
  automated_nat_gateway_dropped_packets_percent_threshold = 0.01
  ...
  automated_alarms = {
    ...
    nat_gateway_dropped_packets = {
      percent = local.automated_nat_gateway_dropped_packets_percent_threshold
    }
...
module "nat_gateway_dropped_packets" {
  source        = "github.com/massdriver-cloud/terraform-modules//aws/cloudwatch-alarm-expression?ref=bafd9d9"
  ...
  md_metadata         = var.md_metadata
  message             = "AWS NAT Gateway ${aws_nat_gateway.private[each.key].id}: Percent Dropped Packets > ${local.alarms.nat_gateway_dropped_packets.percent}%"
  alarm_name          = "${aws_nat_gateway.private[each.key].id}-highDroppedPacketCount"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
  threshold           = local.alarms.nat_gateway_dropped_packets.percent

  metric_queries = {
    "percent_dropped_packets" = {
      expression  = "dropped_packets/(packets_in_from_dest+packets_in_from_source+packets_out_to_dest+packets_out_to_source+1)*100"
      label       = "Percent Dropped Packets"
      return_data = "true"
    },
    "dropped_packets" = {
      metric = {
